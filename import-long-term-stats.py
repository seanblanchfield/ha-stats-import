...
def import_statistics(source_db, target_db, batch_size=1000, dry_run=False):
    ...
    (id, created, metadata_id, start, mean, min, max, last_reset, state, sum, created_ts, start_ts, last_reset_ts)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    
    # Get the total number of rows
    source_cur.execute("SELECT COUNT(*) FROM statistics")
    ...
    while True:
        source_cur.execute(f"SELECT * FROM statistics LIMIT {batch_size} OFFSET {offset}")
